   			...
   			<div class="row">
		    	<div class="col-lg-12">
		          	...
		          	<div class="card">
	            		...
	            		<div class="card-body">
	            			<form action="{{route('salary.save')}}" method="POST" enctype="multipart/form-data">
	            				{{ csrf_field() }}
		            			<div class="form-group row ">
		            				<div class="col-sm-4">
										<label>Employee Name</label>
										<select class="form-control form-control-rounded" name="employee_id" required>
											<option value="{{ old('role') }}"> -- Select The Employee -- </option>
											<option value=""> </option>
                                    ...
                                    <div class="col-sm-4">
										...
										<select name="over_time" class="form-control 
											...
											<option value="">-- Select The Over Time -- </option>
											<option value="{{old('over_time')}}"> {{old('over_time')}}</option>
                                            <option value=""> </option>
                                    ...
                                    <div class="col-sm-4">
										<label>Rate</label>
										<input type="number" class="form-control form-control-rounded" value="{{ old('rate') }}" 
											name="rate" required placeholder="Enter The Rate">
					                    <span style="color: red">** This Field is Required **</span>
					                     @if ($errors->has('rate'))
                                            <div class="alert alert-danger alert-dismissible" role="alert">
										        <button type="button" class="close" data-dismiss="alert">&times;</button>
                                    ...
                                    <div class="col-sm-4">
										...
										<select name="hours" class="form-control 
											...
											<option value="">-- Select The Work Hours -- </option>
											<option value="{{old('hours')}}"> {{old('hours')}}</option>
                                            <option value=""> </option>
                                            @for($i =0; $i <=15; $i++)
									...
									<div class="col-sm-4">
										...
										<select name="weekly" class="form-control 
											...
											<option value="{{old('weekly')}}"> {{old('weekly')}}</option>
											<option value=""> </option>
											@for($k =0; $k <=7; $k++)
					                ...
					                <div class="col-sm-12" align="center">
					                    <button type="submit" class="btn btn-success btn-lg btn-block">ADD THE SALARY </button>
					                </div>
		    	...
		    	<div class="col-lg-12">
		          	<div class="card">
						@if(auth()->user()->hasRole('Administrator') 
							...
							@else
								<div class="card-header"><i class="fa fa-table"></i> List of Saved Employee Salaries in All Ware House</div>
								<div class="card-body">
												...
												@foreach($salary as $salaries)
													<tr>
														...
														<td>@if(auth()->user()->hasRole('Administrator') OR(
															...
															@endif
															@can('salary-edit')
																<a href="{{route('salary.edit', $salaries->salary_id)}}" 
											...
											@foreach($sal as $sala)
												<tr>
													...
													<td>@if(auth()->user()->hasRole('Administrator') OR(
														...
														@endif
														@can('salary-edit')
															<a href="{{route('salary.edit', $sala->salary_id)}}" 
																onclick="return(confirmToEdit());" class="btn btn-success">
																<i class="fa fa-pencil"></i>Edit
															</a>
														@endcan 
